...
class BookScanning(Interface):
    ...
    def get_descriptions(self):
        ...
        for index, describe in enumerate(nested_vert):
            ...
            for i, number in enumerate(lib_desc):
                ...
                if i == 1:
                    if number > 10 ** 5:
                        print(error_handles[15] + " " + str(index) + error_handles[13] + str(10 ** 5) + str(
                            error_handles[14]))
                        return
                ...
                if i == 2:
                    if number > 10 ** 5:
                        print(
                            error_handles[16] + str(index) + error_handles[13] + str(10 ** 5) + str(error_handles[14]))
                        return
    ...
    def book_selection(self):
        ...
        master_book_dicts = [{key: dict_} for key, dict_ in
                             zip(selected_libraries, list_of_candbooks)]  # dicts,keys from nest
    ...
    def scanning_schedule(self):
        """
        Calculates a scanning schedule that maximises the number
        of books on hand per library scaled by the rate
        at which a library can send books.
        ...
        for i in range(len(candidate_books)):
            ...
            book_list = [x for x in book_list if x not in self.books_scanned]  # Filters books already scanned
    ...
    def submission_file(self):
        ...
        for i, library in enumerate(selected_libraries):
            ...
            write_string = str(library) + ' ' + books_no + '\n' + schedule_string + '\n'
